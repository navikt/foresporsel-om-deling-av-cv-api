package no.nav.arbeid.dir.utils
...
fun <T : Any> meterCall(requestUrl: String,
                        requestShortName: String? = null,
                        call: Supplier<T>): T {
...
fun meterOutbound(requestUrl: String, requestShortName: String?, status:String = "200", forbruktTid: Long) {
    val target = requestShortName ?: Regex("https?://([^/]*)/?.*").matchEntire(requestUrl)?.groups?.drop(1)?.firstOrNull()?.value ?: requestUrl
